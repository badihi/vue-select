		...
		filter: {
			...
			default(options, search) {
				return options.filter(option => {
					...
					return (
						this.filterBy(option, label, search) && !option.header
					);
				});
			}
		},
	...
	methods: {
			...
			if (typeof this.$refs.openIndicator !== "undefined") {
				toggleTargets.push(
					...
					...Array.prototype.slice.call(
						this.$refs.openIndicator.$el.childNodes
					)
				);
			}
		...
		findOptionFromReducedValue(value) {
			return (
				this.options.find(
					option =>
						JSON.stringify(this.reduce(option)) ===
		...
		optionExists(option) {
			return this.optionList.some(opt => {
				if (
					typeof opt === "object" &&
					this.getOptionLabel(opt) === option
				) {
					return true;
				} else if (opt === option) {
					return true;
        ...
        /**
		 * Converts class string to a object to be appended to other classes
		 * @param classStr The string of class
		 */
		prepareClasses(classStr) {
            const ret = {};
            console.log(classStr);
			classStr && classStr.split && classStr.split(' ').forEach(e => (ret[e] = true));
			return ret;
		},
	},
